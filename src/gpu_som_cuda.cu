...
#include "../include/gpu_self_organising_maps_CUDA.hpp"
...
int main(int argc, char* argv[]) {
    ...
    try {
        if (argc == 5) {
            gpu_self_organising_maps::gpu_self_organising_maps<float> s(stoi(argv[1]), stoi(argv[2]), string(argv[3]), string(argv[4]), "euclidean");

            s.run();
        }
        else {
            gpu_self_organising_maps::gpu_self_organising_maps<float> s(stoi(argv[1]), stoi(argv[2]), string(argv[3]), string(argv[4]), string(argv[5]));
